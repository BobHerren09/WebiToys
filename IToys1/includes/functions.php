...
function tao_don_hang($conn, $khach_hang_id, $ho_ten, $email, $dien_thoai, $dia_chi, $ghi_chu = '')
{
    ...
    if ($stmt->execute()) {
        ...
        cap_nhat_so_luong_khi_dat_hang($conn, $don_hang_id);
...
function cap_nhat_trang_thai_don_hang($conn, $don_hang_id, $trang_thai)
{
    ...
    if ($stmt->execute()) {
        // Cập nhật số lượng sản phẩm khi thay đổi trạng thái
...
/**
 * Hàm cập nhật số lượng sản phẩm khi tạo đơn hàng mới
 * @param mysqli $conn Kết nối cơ sở dữ liệu
 * @param int $don_hang_id ID đơn hàng
 * @return bool Trả về true nếu cập nhật thành công
 */
function cap_nhat_so_luong_khi_dat_hang($conn, $don_hang_id)
{
    // Lấy chi tiết đơn hàng
    $sql = "SELECT ctdh.san_pham_id, ctdh.so_luong FROM chi_tiet_don_hang ctdh WHERE ctdh.don_hang_id = ?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("i", $don_hang_id);
    $stmt->execute();
    $result = $stmt->get_result();
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        $sql_update = "UPDATE san_pham SET so_luong = so_luong - ? WHERE id = ?";
        $stmt_update = $conn->prepare($sql_update);
        $stmt_update->bind_param("ii", $row['so_luong'], $row['san_pham_id']);
        $stmt_update->execute();
    }
...
/**
 * Hàm cập nhật số lượng sản phẩm khi thay đổi trạng thái đơn hàng
 * @param mysqli $conn Kết nối cơ sở dữ liệu
 * @param int $don_hang_id ID đơn hàng
 * @param int $trang_thai_moi Trạng thái mới của đơn hàng
 * @param int $trang_thai_cu Trạng thái cũ của đơn hàng
 * @return bool Trả về true nếu cập nhật thành công
 */
function cap_nhat_so_luong_san_pham($conn, $don_hang_id, $trang_thai_moi, $trang_thai_cu)
{
    // Nếu đơn hàng mới (trạng thái = 0), trừ số lượng sản phẩm
    if ($trang_thai_moi == 0 && $trang_thai_cu != 0) {
        // Lấy chi tiết đơn hàng
        $sql = "SELECT ctdh.san_pham_id, ctdh.so_luong FROM chi_tiet_don_hang ctdh WHERE ctdh.don_hang_id = ?";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("i", $don_hang_id);
    ...
    // Nếu đơn hàng bị hủy (trạng thái = 4), hoàn lại số lượng sản phẩm
    if ($trang_thai_moi == 4 && $trang_thai_cu != 4) {
        // Lấy chi tiết đơn hàng
        $sql = "SELECT ctdh.san_pham_id, ctdh.so_luong FROM chi_tiet_don_hang ctdh WHERE ctdh.don_hang_id = ?";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("i", $don_hang_id);
    ...
    // Nếu đơn hàng từ trạng thái hủy (4) chuyển sang trạng thái khác, trừ lại số lượng
    if ($trang_thai_cu == 4 && $trang_thai_moi != 4) {
        // Lấy chi tiết đơn hàng
        $sql = "SELECT ctdh.san_pham_id, ctdh.so_luong FROM chi_tiet_don_hang ctdh WHERE ctdh.don_hang_id = ?";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("i", $don_hang_id);
...
/**
 * Hàm kiểm tra số lượng sản phẩm trước khi đặt hàng
 * @param mysqli $conn Kết nối cơ sở dữ liệu
 * @param array $gio_hang Mảng giỏ hàng
 * @return array Mảng các sản phẩm hết hàng
 */
function kiem_tra_so_luong_san_pham($conn, $gio_hang)
{
    $san_pham_het_hang = array();

    foreach ($gio_hang as $san_pham_id => $so_luong) {
        // Kiểm tra số lượng sản phẩm trong kho
        $sql = "SELECT so_luong, ten_san_pham FROM san_pham WHERE id = ?";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("i", $san_pham_id);
        $stmt->execute();
        $result = $stmt->get_result();
        ...
        if ($result->num_rows > 0) {
            $row = $result->fetch_assoc();
            if ($row['so_luong'] < $so_luong) {
                $san_pham_het_hang[] = array(
                    'id' => $san_pham_id,
                    'ten_san_pham' => $row['ten_san_pham'],
                    'so_luong_con' => $row['so_luong'],
                    'so_luong_dat' => $so_luong
                );
            }
        }
    }

    return $san_pham_het_hang;
}
?>